...
Next task:# InvoiceFlow – Project Context
...
- Always provide downloadable link for the updated PROJECT_CONTEXT.md after the step is confirmed working
- Keep chat small by preferring download links over pasting large full files into chat
...
Implemented:
- improved Add Item dialog so typing in description suggests existing catalog items
- added catalog lookup while typing inside the item dialog
- tapping a suggestion auto-fills:
  - description
  - quantity
  - unit price
- kept manual item entry fully supported
- kept dialog layout mobile-safe and stable
- preserved current add item flow
- preserved edit item flow
- preserved bookmark-to-catalog flow
- preserved catalog bottom sheet behavior
- kept PDF logic untouched
- kept Arabic PDF untouched in this step

### Important fixes/decisions inside Step 32:
- solution was kept inside the existing item dialog flow for safety
- avoided risky changes to PDF generation and invoice save logic
- final dialog implementation was simplified to avoid layout/render crashes
- autocomplete suggestions are shown in a safe dialog-friendly layout
- maintained compatibility with the existing `CreateInvoiceScreen` integration
- kept Step 19–31 flows stable

Final result:
- invoice item entry is faster
- users can reuse catalog items while typing
- item details auto-fill correctly from catalog suggestions
- manual workflow still works normally
- catalog autocomplete is stable and working
...
- partial payment display in preview and PDF
- improved card readability for total / paid / remaining
- catalog autocomplete while typing item description
...
- mobile-safe catalog sheet
- linked item sync in active invoice form
- autocomplete suggestions in Add Item dialog
...
Goal:
- further improve invoice item entry speed after catalog autocomplete
- reduce repeated typing for frequently used items
- keep the current create/edit flow stable

Suggested scope:
- smarter defaults for quantity and unit price when appropriate
- faster editing workflow for invoice items
